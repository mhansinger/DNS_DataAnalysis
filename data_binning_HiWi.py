...
class data_binning_PDF(object):
    ...
    # @jit(nopython=True)
    def run_analysis_wrinkling(self, filter_width=8, interval=2, c_min_thresh=0.01, c_max_thresh=0.99, histogram=False,
                               write_csv=False):
        ...
        for k in range(self.filter_width - 1, self.Nx, self.interval):
            for j in range(self.filter_width - 1, self.Nx, self.interval):
                for i in range(self.filter_width - 1, self.Nx, self.interval):
                    ...
                    # print(count)
                    # count+=1
                    obj = delayed(self.compute_loop_stuff)(i, j, k, c_min_thresh, c_max_thresh, histogram)
                    obj.compute()

        # write data to csv file
        filename = join(self.case, 'filter_width_' + str(self.filter_width) + '.csv')
        dataArray_pd = pd.DataFrame(data=self.dataArray_np,
                                    columns=['c_bar', 'wrinkling', 'omega_DNS_Klein', 'omega_DNS_Pfitzner',
                                             'omega_model', 'c_plus', 'c_minus'])
        dataArray_pd.to_csv(filename, index=False)
        print('Data has been written.')

    def compute_loop_stuff(self, i, j, k, c_min_thresh, c_max_thresh, histogram):
        # this is the current data cube which constitutes the LES cell
    ...
    def plot_histograms(self, c_tilde, this_rho_c_reshape, this_rho_reshape, this_RR_reshape_DNS):
        ...
        fig_name = join(self.output_path, 'c_bar_%.4f_wrinkl_%.3f_filter_%i_%s.png' % (
    ...
    def plot_histograms_intervals(self, c_tilde, this_rho_c_reshape, this_rho_reshape, c_bar, this_RR_reshape_DNS,
                                  wrinkling=1):
        ...
        fig_name = join(self.output_path,
                        'c_bar_%.4f_wrinkl_%.3f_filter_%i_%s.png' % (c_bar, wrinkling, self.filter_width, self.case))
        plt.savefig(fig_name)
    ...
    def compute_wrinkling_RR(self, i, j, k, histogram):
        ...
        this_RR_reshape_DNS_Pfitz = 18.97 * ((1 - self.alpha * (1 - this_c_reshape))) ** (-1) * (
                    1 - this_c_reshape) * np.exp(exponent)
        ...
        for l in range(self.filter_width):
            for m in range(self.filter_width):
                for n in range(self.filter_width):
                    this_DNS_gradX[l, m, n] = (this_rho_c_data[l + 1, m, n] / this_rho_data[l + 1, m, n] -
                                               this_rho_c_data[l - 1, m, n] / this_rho_data[l - 1, m, n]) / (2 * width)
                    this_DNS_gradY[l, m, n] = (this_rho_c_data[l, m + 1, n] / this_rho_data[l, m + 1, n] -
                                               this_rho_c_data[l, m - 1, n] / this_rho_data[l, m - 1, n]) / (2 * width)
                    this_DNS_gradZ[l, m, n] = (this_rho_c_data[l, m, n + 1] / this_rho_data[l, m, n + 1] -
                                               this_rho_c_data[l, m, n - 1] / this_rho_data[l, m, n - 1]) / (2 * width)
                    # compute the magnitude of the gradient
                    this_DNS_magGrad_c[l, m, n] = np.sqrt(
                        this_DNS_gradX[l, m, n] ** 2 + this_DNS_gradY[l, m, n] ** 2 + this_DNS_gradZ[l, m, n] ** 2)
    ...
    def compute_c_minus(self):
        ...
        self.c_minus = (np.exp(self.c_bar * this_delta_0 * self.Delta_LES) - 1) / (
